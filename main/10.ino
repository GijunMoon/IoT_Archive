...
// 변수 선언
float inhum;            // 실내 습도 값
float intemp;           // 실내 온도 값
float outhum;           // 실외 습도 값
float outtemp;          // 실외 온도 값
...
const char* pm25ValueP;
const char* readingP;
const char* rainValueP;
const char* discomfortIndex1P;
const char* discomfortIndex2P;
...
float humValues[INTERVAL] = { 0 };    // 실내 습도 값 배열
float tempValues[INTERVAL] = { 0 };   // 실내 온도 값 배열
float hum2Values[INTERVAL] = { 0 };   // 실외 습도 값 배열
float temp2Values[INTERVAL] = { 0 };  // 실외 온도 값 배열
...
void printSensorReadings() {  //아두이노 센서 값 출력 함수 정의
  ...
  Serial.print(outhum, 1);  //실외 습도값 소수점 이하 1자리만 출력
  Serial.print(F(","));     //습도 단위 출력

  // Serial.print(F("Temperature 1: "));  //현재 실외 습도
  Serial.print(outtemp, 1);  //실외 온도값 소수점 이하 1자리만 출력
  Serial.print(F(","));      //온도 단위 출력

  //Serial.print(F("Humidity 2: "));  // 현재 실내 습도
  Serial.print(inhum, 1);  //실내 습도값 소수점 이하 1자리만 출력
  Serial.print(F(","));    //습도 단위 출력

  // Serial.print(F("Temperature 2: "));  // 현재 실내 온도
  Serial.print(intemp, 1);  //실내 온도값 소수점 이하 1자리만 출력
  Serial.print(F(","));     //온도 단위 출력



  if (reading >= 200) {                         // 조도 값이 400 이상일 경우
    readingP = "Very Bright";                    // "매우 밝음" 출력
  } else if (reading >= 20 && reading < 200) {  // 조도 값이 200 이상, 400 미만일 경우
    readingP = "Normal Light";                  //"보통" 출력
  } else if (reading < 20) {                    // 조도 값이 200 미만일 경우
    readingP = "Very Dark";                     //"매우 어두움" 출력
  }
  // Serial.print(F("Light Level: "));  // 현재 조도
  Serial.print(readingP);  // 조도값 소수점 이하 2자리까지 출력
  Serial.print(F(","));

  if (rainValue < 500) {                             // 빗물 감지 값이 500 미만일 ruddn
    rainValueP = "Heavy Rain";                       //"Heavy Rain" 출력
  } else if (rainValue >= 500 && rainValue < 900) {  // 빗물 감지 값이 500 이상, 900 미만일 경우
    rainValueP = "Moderate Rain";                    //"Moderate Rain" 출력
  } else {                                           // 빗물 감지 값이 900 이상일 경우
    rainValueP = "No Rain";                          //"No Rain" 출력
  }

  //  Serial.print(F("Rain Level: "));  // 현재 빗물
  Serial.print(rainValueP);  //빗물값 출력
  Serial.print(F(","));

  if (pm25Value >= 0 && pm25Value < 30) {
    pm25ValueP = "dust very good";  // 미세먼지 상태 매우 좋음
  } else if (pm25Value >= 30 && pm25Value < 80) {
    pm25ValueP = "dust normal";  // 미세먼지 상태 보통
  } else if (pm25Value >= 80 && pm25Value < 150) {
    pm25ValueP = "dust bad";  // 미세먼지 상태 나쁨
  } else if (pm25Value >= 150) {
    pm25ValueP = "dust bad";  // 미세먼지 상태 매우 나쁨
  }

  // 상태 메시지 출력
  Serial.print(pm25ValueP);
  Serial.print(F(","));
  // 미세먼지 농도 출력 (소수점 이하 2자리까지)

  float discomfortIndex1 = calculateDiscomfortIndex(outtemp, outhum);  //실외 불쾌지수 저장
  float discomfortIndex2 = calculateDiscomfortIndex(intemp, inhum);    //실내 불쾌지수 저장



  if (discomfortIndex1 >= 80) {                                // 불쾌지수 값이 80 이상일 경우
    discomfortIndex1P = "very high discomfort OUT";  //"불쾌지수 매우 높음" 출력
  } else if (discomfortIndex1 >= 75) {                         // 불쾌지수 값이 75 이상일 경우
    discomfortIndex1P = "high discomfort OUT";        //"불쾌지수 높음" 출력
  } else if (discomfortIndex1 >= 68) {                         // 불쾌지수 값이 68 이상일 경우
    discomfortIndex1P = "comfort discomfort OUT";            //"쾌적함" 출력
  } else {                                                     // 불쾌지수 값이 68 미만일 경우
    discomfortIndex1P = "very comfort discomfort OUT";      //"매우 쾌적함" 출력
  }
  ...
  if (discomfortIndex2 >= 80) {                                  // 불쾌지수 값이 80 이상일 경우
    discomfortIndex2P = "very high discomfort IN";  //"불쾌지수 매우 높음" 출력
  } else if (discomfortIndex2 >= 75) {                           // 불쾌지수 값이 75 이상일 경우
    discomfortIndex2P = "high discomfort IN";        //"불쾌지수 높음" 출력
  } else if (discomfortIndex2 >= 68) {                           // 불쾌지수 값이 68 이상일 경우
    discomfortIndex2P = "comfort discomfort IN";             //"쾌적함" 출력
  } else {                                                       // 불쾌지수 값이 68 미만일 경우
    discomfortIndex2P = "very comfort discomfort IN";       //"매우 쾌적함" 출력
  }
...
// 창문제어함수
void controlServoBasedOnAverage(float avgTemp, float avgHum, float avgTemp2, float avgHum2) {  //실외2, 실내2
 ...
 // Serial.print(F("10avg discomfort OUT: "));  //현재 실외 불쾌지수 평균
  Serial.print(discomfortIndex1, 2);   
  Serial.print(F(","));                     //실외 불쾌지수 평균 소수점 이하 2자리까지 출력

  //Serial.print(F("10avg discomfort IN: "));  //현재 실내 불쾌지수 평균
  Serial.print(discomfortIndex2, 2);   
  Serial.print(F(","));                     //실내 불쾌지수 평균 소수점 이하 2자리까지 출력
  ...
  if (avgTemp2 > userHot || avgHum2 > userHumi || discomfortIndex2 > discomfortIndex1) {
    ...
    Serial.println(F("Door: Open"));  //문을 열라는 문자 출력
    ...
    Serial.println(F("Door: Close"));    //문을 닫으라는 문자 출력
  } else {                               //위의 경우에 모두 해당하지 않는다면
                                         // myservo.write(90);  // 중립 위치
    Serial.println(F("Door: Neutral"));  //문이 중립이라는 문자 출력
  }
...
void loop() {                    //계속 반복
  ...
  inhum = DHT.humidity;             //실외 hum 값 저장
  intemp = DHT.temperature;         //실외temp 값 저장
  delay(100);

  chk = DHT.read21(DHT21_PIN2);  // 두 번째 DHT-21 센서에서 데이터 읽기
  outhum = DHT.humidity;         //실내 hum 값 저장
  outtemp = DHT.temperature;     //실내temp 값 저장
  ...
  if (now - lastSampleTime >= 6000) {      // 1분 = 60,000밀리초
    ...
    temp2Values[currentIndex] = outtemp;   // 실내 온도 값 저장
...
void loop_10avg() {                              //// 10분 평균 계산 및 서보 제어 함수 정의
 // Serial.println(F("Calculating averages..."));  // 평균 계산 문구 출력
  ...
  // 디버그용 평균 값 출력
  //Serial.print(F("Avg outhum: "));     // 실외 습도 평균 표시 출력
  Serial.print(avgHum);                   
  Serial.print(F(","));            //실외 습도 평균 값 출력
  //Serial.print(F("Avg outtemp: "));  // 실외 온도 평균 표시 출력
  Serial.print(avgTemp); 
  Serial.print(F(","));                             // 실외 온도 평균 값 출력
  //Serial.print(F("Avg inhum: "));     // 실내 습도 평균 표시 출력
  Serial.print(avgHum2);  
  Serial.print(F(","));                            // 실내 습도 평균 값 출력
  //Serial.print(F("Avg intemp: "));  // 실내 온도 평균 표시 출력
  Serial.print(avgTemp2); 
  Serial.print(F(","));                            // 실내 온도 평균 값 출력
  //Serial.print(F("Avg light: "));             // 조도 평균 표시 출력
  Serial.print(avgLight); 
  Serial.print(F(","));                            // 조도 평균 값 출력
  //Serial.print(F("Avg Rain: "));              // 빗물 감지 평균 표시 출력
  Serial.print(avgRain);  
  Serial.print(F(","));                            // 빗물 감지 평균 값 출력
  //Serial.print(F("Avg PM2.5: "));             // 미세먼지 농도 평균 표시 출력
  Serial.print(avgPM25);                              // 미세먼지 농도 평균 값 출력
  Serial.print(F(",")); 
  delay(100);  //0.1초 동안 프로그램 일시 중지
  // PDLC 제어
