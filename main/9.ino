...
void printSensorReadings() {  //아두이노 센서 값 출력 함수 정의
  ...
  if (pm25Value >= 0 && pm25Value < 30) {
    pm25ValueP = "Fine dust concentration is very good";  // 미세먼지 상태 매우 좋음
  } else if (pm25Value >= 30 && pm25Value < 80) {
    pm25ValueP = "Fine dust concentration is normal";  // 미세먼지 상태 보통
  } else if (pm25Value >= 80 && pm25Value < 150) {
    pm25ValueP = "Fine dust concentration is bad";  // 미세먼지 상태 나쁨
  } else if (pm25Value >= 150) {
    pm25ValueP = "Fine dust concentration is very bad";  // 미세먼지 상태 매우 나쁨
  }
  ...
  if (discomfortIndex1 >= 80) {                                // 불쾌지수 값이 80 이상일 경우
    discomfortIndex1P = "The discomfort index is very high!";  //"불쾌지수 매우 높음" 출력
  } else if (discomfortIndex1 >= 75) {                         // 불쾌지수 값이 75 이상일 경우
    discomfortIndex1P = "The discomfort index is high";        //"불쾌지수 높음" 출력
  } else if (discomfortIndex1 >= 68) {                         // 불쾌지수 값이 68 이상일 경우
    discomfortIndex1P = "It's comfortable outside";            //"쾌적함" 출력
  ...
  if (discomfortIndex2 >= 80) {                                  // 불쾌지수 값이 80 이상일 경우
    discomfortIndex2P = "The discomfort index is very high!\n";  //"불쾌지수 매우 높음" 출력
  } else if (discomfortIndex2 >= 75) {                           // 불쾌지수 값이 75 이상일 경우
    discomfortIndex2P = "The discomfort index is high\n";        //"불쾌지수 높음" 출력
  } else if (discomfortIndex2 >= 68) {                           // 불쾌지수 값이 68 이상일 경우
    discomfortIndex2P = "The interior is comfort\n";             //"쾌적함" 출력
  } else {                                                       // 불쾌지수 값이 68 미만일 경우
