...
float userHot = 26;     // 기본 설정 기본 더운 온도
float userCool = 24.0;  // 기본 설정 기본 추운 온도
float userDust = 81;    // 기본 설정 기본 미세먼지 농도
float userLight = 400;  // 기본 설정 기본 조도
float reading;          // 조도 값
int rainValue;          // 빗물 감지 값
float pm25Value;        // PM2.5 미세먼지 농도 값
String doorState = "door Closed"; // 문 상태 값
...
void processUserInput() {  //설정 재입력 함수 정의
  if (Serial.available() > 0) {  //입력 데이터에 정보가 있다면
    ...
    if (inputCommand.equalsIgnoreCase("설정")) {  //설정이라는 단어를 입력받은 경우
...
void printSensorReadings() {  //아두이노 센서 값 출력 함수 정의
  ...
  float discomfortIndex1 = calculateDiscomfortIndex(temp, hum);  //실외 불쾌지수 저장
  float discomfortIndex2 = calculateDiscomfortIndex(temp2, hum2);  //실내 불쾌지수 저장
  ...
  } else if (doorState == "door Opened"){
    Serial.print(F("Door: Opened"));
    Serial.println(doorState);
  } else if (doorState == "door Netural"){
    Serial.print(F("Door: Netural"));
    Serial.println(doorState);
  }
...
// 서보 제어 함수
void controlServoBasedOnAverage(float avgTemp1, float avgHum1, float avgTemp2, float avgHum2) {  //센서에 따른 서보 제어 함수 정의
  float discomfortIndex1 = calculateDiscomfortIndex(avgTemp1, avgHum1);  //실외 불쾌지수 평균 저장
  ...
  if (avgTemp2 > userHot || avgHum2 > userHumi || discomfortIndex1 < discomfortIndex2 ) {  
    //사용자 설정 온도보다 실내 온도가 높거나 설정 습도보다 실내 습도가 높거나 실외 불쾌지수가 실내 불쾌지수보다 낮거나 조건을 하나 이상 충족할 경우
    myservo.write(180);  // 서보를 열림 위치로 회전
    //Serial.println(F("door Opened"));  //문을 열라는 문자 출력
    doorState = "door Opened";
  } else if (temp2 < userCool || pm25Value > userDust || (discomfortIndex1 > discomfortIndex2 || discomfortIndex1 >= 70 || temp >= 30 || hum >= 80 || rainValue < 900 || pm25Value >= 81)) {
...
void loop() {  //계속 반복
  ...
  // PDLC 제어
  if (reading >= 50) {  //50보다 조도값이 크거나 같을 경우
    for (pos = 65; pos >= 0; pos -= 1) {  // 65도에서 0도까지 이동
      myservo.write(pos);                 // 서보를 'pos' 위치로 이동
      break;                              // 서보가 위치에 도달할 때까지 15ms 대기
    }
  ...
  /*1 : 잠김
    ...
    delay(1000);  //1초 동안 프로그램 일시 중지
    digitalWrite(Relaypin, LOW);// 다시 릴레이 핀을 LOW로 설정하여 잠금 상태로 전환
  } 
}
...
void loop_10avg() {  //// 10분 평균 계산 및 서보 제어 함수 정의
  ...
  float avgHum = calculateAverage(humValues, INTERVAL);  // 실외 습도 평균 값 계산
  float avgTemp = calculateAverage(tempValues, INTERVAL); // 실외 온도 평균 값 계산
  float avgHum2 = calculateAverage(hum2Values, INTERVAL); // 실내 습도 평균 값 계산
  float avgTemp2 = calculateAverage(temp2Values, INTERVAL); // 실내 온도 평균 값 계산
